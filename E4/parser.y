...
elemento: declaracao_variavel	{    if ($1 != NULL) asd_free($1);
                                   $$ = NULL; }
        | declaracao_funcao	{ $$ = $1; }
        ;
...
sequencia_comandos:   comando                       { $$ = $1; }
                    | comando sequencia_comandos    { 
                                    if ($1 != NULL) {
                                        ...
                                        /* Se $1 é NULL (var local), a sequência é só o resto ($2) */
                                        $$ = $2;
                                    }
                                }
                    ;
...
expr_logica_ou: expr_logica_ou '|'
expr_logica_e	{ $$ = asd_new_binary_op("|", $1, $3); }
          | expr_logica_e { $$ = $1; }
          ;
expr_logica_e: expr_logica_e '&' expr_igualdade { $$ = asd_new_binary_op("&", $1, $3); }
          | expr_igualdade { $$ = $1; }
          ;
expr_igualdade: expr_igualdade TK_OC_EQ expr_relacional { $$ = asd_new_binary_op("==", $1, $3); } 
          | expr_igualdade TK_OC_NE expr_relacional { $$ = asd_new_binary_op("!=", $1, $3); }
          | expr_relacional { $$ = $1; }
          ;

expr_relacional: expr_relacional '<' expr_aditiva { $$ = asd_new_binary_op("<", $1, $3); }
          | expr_relacional '>' expr_aditiva		{ $$ = asd_new_binary_op(">", $1, $3); }
          | expr_relacional TK_OC_LE expr_aditiva { $$ = asd_new_binary_op("<=", $1, $3); }
          | expr_relacional TK_OC_GE expr_aditiva { $$ = asd_new_binary_op(">=", $1, $3); }
          | expr_aditiva		{ $$ = $1;}
          ;

expr_aditiva: expr_aditiva '+' expr_multiplicativa { $$ = asd_new_binary_op("+", $1, $3); }
          | expr_aditiva '-' expr_multiplicativa { $$ = asd_new_binary_op("-", $1, $3); }
          | expr_multiplicativa	{ $$ = $1; }
          ;

expr_multiplicativa: expr_multiplicativa '*' expr_unaria { $$ = asd_new_binary_op("*", $1, $3); }
          | expr_multiplicativa '/' expr_unaria { $$ = asd_new_binary_op("/", $1, $3); }
          | expr_multiplicativa '%' expr_unaria	{ $$ = asd_new_binary_op("%", $1, $3); }
          | expr_unaria	{ $$ = $1; }
          ;

expr_unaria: '+' fator { $$ = asd_new_unary_op("+", $2); }
           | '-' fator { $$ = asd_new_unary_op("-", $2); }
           | '!' fator { $$ = asd_new_unary_op("!", $2); }
           | fator     { $$ = $1; }
           ;
