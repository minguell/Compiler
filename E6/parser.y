...
declaracao_funcao: cabecalho_funcao '[' sequencia_comandos ']' {
        ...
        // --- ALTERAÇÃO: Gerar o Label da função ---
        // O simulador exige que labels comecem com 'L'. Adicionamos 'L' ao nome.
        char func_label[256];
        sprintf(func_label, "L%s", $1->label); // Ex: main vira Lmain

        // Cria o nó do label
        ILOC_Node *label_node = new_label_node(func_label);
        ILOC_List *label_list = new_iloc_list(label_node);

        // Prepend o label ao corpo da função ($3)
        if ($3 != NULL) {
           asd_add_child($$, $3);
           // Código = Label + Corpo
           $$->code = safe_concat(label_list, $3->code);
           $3->code = NULL;
        } else {
           // Função vazia: Código = Label
           $$->code = label_list;
        }
   ...
   | cabecalho_funcao '[' ']' {
        // Lógica similar para função vazia sem comandos
        $$ = $1;
        
        char func_label[256];
        sprintf(func_label, "L%s", $1->label);
        
        $$->code = new_iloc_list(new_label_node(func_label));
